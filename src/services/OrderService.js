...
export const getOrderByUserId = async (id,access_token) => {
  const res = await axiosJWT.get(`${process.env.REACT_APP_API_URL}/order/get-all-order-user/${id}`, {
      headers: {
          token: `Bearer ${access_token}`,
      }
  })
...
export const cancelOrder = async (id, access_token, orderItems,orderPetItems, userId ) => {
  const data = {orderItems,orderPetItems, orderId: id}
